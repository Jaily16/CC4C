...
if (-not $resolvedBackupPath.StartsWith(
    ...
    -not [System.IO.Path]::GetFileName($resolvedBackupPath).StartsWith(
        'cc4c-v3-aspect2-cc4c_test-',
        [System.StringComparison]::OrdinalIgnoreCase) -or
    [System.IO.Path]::GetExtension($resolvedBackupPath) -ne '.sql') {
    throw 'Backup must be an aspect-two cc4c_test SQL dump under the ignored temp directory.'
}
...
$safeRestorePath = Join-Path $allowedBackupDirectory 'cc4c-v3-aspect2-recovery-import.sql'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to inspect $targetDatabase. Run database/test-database-recovery-setup.sql as an administrator first."
    }
