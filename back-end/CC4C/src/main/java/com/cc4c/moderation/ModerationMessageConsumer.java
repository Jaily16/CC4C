...
class ModerationMessageConsumer {
    ...
    private void sendReviewed(MessageEnvelope envelope, byte[] plaintext) {
        ...
        mailSender.sendText(
                ...
                "您的博客《" + payload.title() + "》（ID：" + payload.blogId() + "）"
                        + (approved ? "已通过审核。" : "未通过本次审核。"));
    }
    ...
    private <T> T read(byte[] plaintext, Class<T> type) {
        try {
            ...
            throw new MessagePayloadException(
                    "INVALID_PAYLOAD", "Blog notification payload cannot be read", exception);
        }
    }
}
