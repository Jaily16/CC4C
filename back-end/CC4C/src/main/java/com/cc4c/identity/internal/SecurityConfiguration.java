...
class SecurityConfiguration {
    ...
    SessionAuthenticationStrategy sessionAuthenticationStrategy(SessionRegistry sessionRegistry) {
        RoleAwareConcurrentSessionStrategy concurrent =
                new RoleAwareConcurrentSessionStrategy(sessionRegistry);
        ChangeSessionIdAuthenticationStrategy fixation =
    ...
    @Bean
    FilterRegistrationBean<RedisFailureResponseFilter> redisFailureResponseFilter(
            SecurityErrorWriter errorWriter) {
    ...
    RedisSerializer<Object> springSessionDefaultRedisSerializer(ObjectMapper source) {
        ...
        objectMapper.registerModules(
                SecurityJackson2Modules.getModules(Cc4cPrincipal.class.getClassLoader()));
        ...
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .requestMatchers(
                                ...
                                "/swagger-ui.html",
                                "/test/**").permitAll()
                        .requestMatchers(HttpMethod.GET, "/csrf", "/auth/session").permitAll()
                        .requestMatchers(HttpMethod.POST, "/users", "/users/login", "/users/email")
                                .permitAll()
                        .requestMatchers(HttpMethod.PUT, "/users/password/forget").permitAll()
                        .requestMatchers(HttpMethod.POST, "/admin/login").permitAll()
                        .requestMatchers(HttpMethod.GET, "/courses/star", "/courses/star/**")
                                .hasRole("USER")
                        .requestMatchers(HttpMethod.POST, "/courses/star/**").hasRole("USER")
                        .requestMatchers(HttpMethod.DELETE, "/courses/star/**").hasRole("USER")
                        ...
                        .requestMatchers(
                                ...
                                "/blogs/draft",
                                "/blogs/collect",
                                "/blogs/collect/**").hasRole("USER")
                        .requestMatchers(HttpMethod.POST, "/blogs/uploadImg")
                                .hasAnyRole("USER", "ADMIN")
                        .requestMatchers(HttpMethod.POST, "/blogs/submit", "/blogs/collect/**")
                                .hasRole("USER")
                        .requestMatchers(HttpMethod.PUT, "/blogs/draft").hasRole("USER")
                        .requestMatchers(HttpMethod.DELETE, "/blogs/**").hasRole("USER")
                        .requestMatchers(HttpMethod.PUT, "/blogs/click/**").permitAll()
