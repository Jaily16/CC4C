...
public record MessagingProperties(
    ...
    public MessagingProperties {
        ...
        if (consumerRetryDelays != null && (consumerRetryDelays.size() != 3
    ...
    public Map<String, byte[]> payloadKeyMap() {
        ...
        for (String entry : payloadKeys.split(";")) {
            ...
            byte[] decoded;
            try {
                decoded = Base64.getDecoder().decode(entry.substring(separator + 1).trim());
    ...
    private static List<String> parseModerationRecipients(String rawRecipients) {
        ...
        if (recipients.isEmpty()
                || recipients.stream().anyMatch(recipient -> !EMAIL.matcher(recipient).matches())) {
