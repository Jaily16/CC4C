...
public class Cc4cTestInfrastructure {
    ...
    private static final Pattern MANAGED_SCHEMA = Pattern.compile(
            "^cc4c_[a-z0-9]+_(?:existing_test|flyway_test)$");
    private static final AtomicBoolean STARTED = new AtomicBoolean();

    private static final SharedMySqlContainer MYSQL =
            new SharedMySqlContainer(DockerImageName.parse("mysql:8.4.11"))
    ...
    private static final String EXISTING_SCHEMA =
            "cc4c_" + RUN_ID + "_existing_test";
    private static final String EMPTY_SCHEMA =
            "cc4c_" + RUN_ID + "_flyway_test";

    static {
        Runtime.getRuntime().addShutdownHook(
                new Thread(() -> STARTED.set(false), "cc4c-testcontainers-shutdown"));
    }
    ...
    public static void ensureStarted() {
        if (STARTED.compareAndSet(false, true)) {
            try {
                Startables.deepStart(Stream.of(MYSQL, SECURITY_REDIS, CACHE_REDIS, RABBIT)).join();
    ...
    public static String securityRedisUrl() {
        ensureStarted();
        return "redis://" + SECURITY_REDIS.getHost() + ":"
                + SECURITY_REDIS.getMappedPort(6379);
    }

    public static String cacheRedisUrl() {
        ensureStarted();
        return "redis://" + CACHE_REDIS.getHost() + ":"
                + CACHE_REDIS.getMappedPort(6379);
    }

    public static String rabbitUrl() {
        ensureStarted();
        return "amqp://" + RABBIT_USERNAME + ":" + RABBIT_PASSWORD + "@"
                + RABBIT.getHost() + ":" + RABBIT.getMappedPort(5672) + "/" + RABBIT_VHOST;
    }
    ...
    public static void recreateManagedSchema(String schema) {
        ...
        try (Connection connection = DriverManager.getConnection(
                mysqlUrl("cc4c_test"), "root", MYSQL_PASSWORD);
             Statement statement = connection.createStatement()) {
            statement.execute("DROP DATABASE IF EXISTS `" + schema + "`");
            statement.execute("CREATE DATABASE `" + schema
                    + "` CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci");
            statement.execute("GRANT ALL PRIVILEGES ON `" + schema
                    + "`.* TO '" + MYSQL_USERNAME + "'@'%'");
    ...
    private static final class SharedMySqlContainer
            extends MySQLContainer<SharedMySqlContainer> {
    ...
    private static final class SharedRedisContainer
            extends GenericContainer<SharedRedisContainer> {
    ...
    private static final class SharedRabbitMqContainer extends RabbitMQContainer {
        ...
        public String getAmqpUrl() {
            return "amqp://" + RABBIT_USERNAME + ":" + RABBIT_PASSWORD + "@"
                    + getHost() + ":" + getAmqpPort() + "/" + RABBIT_VHOST;
        }
