...
> 状态：方面一、二已完成；方面三已完成本机启动、最小功能闭环、V6 会话重启恢复和三端停止，验收限制见第八节。方面四至六尚未开始。各方面独立提交与最终 Git 验收以对应交接结果为准。
>
...
- [x] 方面三：直接命令启动、浏览器最小闭环及三端停止完成；实际结果和限制见第八节。
- [ ] 方面四：文档收敛与资料迁移，未开始。
...
方面三在现有 D:\codex\CC4C_v5 的本地 v6/readability 分支实施，唯一父提交锁定为 071a320cdbe55d7e09fcc503a94eda4d29ca6c02，基线 tree 为 97a34b609af8762f24bfb3d1f08ebd52c451a003，383 个受控文件，工作树干净且无 upstream。执行记录独立保存于 D:\codex\CC4C_v2\temp\v6-backup\20260908-aspect3-071a320c；名称沿用批准计划，实际功能验证和停止日期为 2026-09-09。

只修改本文和 [本机运行手册](operations/host-runbook.md)。不改变源码、配置、版本、依赖或目录布局，不实施文档收敛。本方面未安装依赖、独立重跑生产构建、执行应用自动化测试、管理员引导或数据库维护；mvn -o spring-boot:run 执行其正常资源与编译生命周期。JAR 切换使用方面二已验收产物，SHA-256 为 CE405C5D1948539843B66BB259A138A8473BE1CFC7D9E45B1141D897400B838C。

用户在三个 PowerShell 7 前台终端启动和停止应用，自行输入账号凭据；实施者只读取页面、端口、进程元数据及批准的公开健康和固定观测结果。不读取 Cookie、Token、本机配置正文、运行日志、数据库/Redis 内容或上传目录。用户自行提供的启动失败诊断只用于定位问题，不将原始日志复制进记录。记录目录只保存基线、操作授权、结果和哈希，方面二检查点保持封存。

用户另行明确授权启动既有 Prometheus，并指定仓库外配置目录。仅检查路径元数据，复用该目录的 prometheus.yml 和 prometheus-data，以 127.0.0.1:9090 启动；未读取或改写私有配置、清理 TSDB、启用管理接口或重配抓取。这是本轮追加授权，不改变项目入口不自动管理中间件的边界。首次进程后来不再监听，用户再次授权后按相同参数启动；最终三端停止不包含 Prometheus。

### 8.2 实际功能证据

| 验收项 | 实际结果 |
| --- | --- |
| 前台入口 | PowerShell 7.6.5、Java 21，离线 Maven、新环境包裹辅助及两端 npm dev 命令均运行成功 |
| 健康与端口 | Maven 和 JAR 两种后端的 health、liveness、readiness 均曾返回 HTTP 200 / UP；前端直连 HTTP 200 |
| 用户读取 | jaily 登录、刷新恢复、首页、课程列表/详情和公开博客读取通过 |
| 管理员读取 | 内容概览、异步消息页查询通过，未重试或忽略任何消息 |
| 身份边界 | USER 管理页面跳转至管理员登录；ADMIN 个人空间跳转至用户登录；业务身份不自动登录观测端 |
| 退出隔离 | 业务退出后观测会话保持；观测退出后管理员业务会话保持；均通过可见页面验证 |
| 收藏往返 | 对课程 ID 1「黑马_20天学会JAVA」逐项确认后新增并取消，两个成功提示及最终未收藏状态通过 |
| 图片映射 | 公开 frontend/src/assets/logo/Logo.png 上传成功，编辑器插入 /blogImg/ URL，页面图片加载尺寸为 1108×540；已有头像亦加载成功，未替换头像 |
| 博客闭环 | jaily 提交合成文章，管理员仅审核该文章；首页公开可见，用户确认收到审核通知邮件 |
| 评论闭环 | 本次文章下发布唯一标识评论，显示 jaily 与成功提示；经单独确认删除后讨论数恢复为 0 |
| 本人删除 | 单独确认删除本次博客，显示成功且列表仅保留原 V5 博客 |
| 观测页面 | 总览、API/JVM、数据库/缓存/安全、消息、告警依赖页均可访问和刷新；六项依赖显示正常，告警载入 20/20 |
| Prometheus | 版本 3.13.2、就绪接口、公开模板和 20 条规则校验通过；Maven 和 JAR 阶段均确认 backend up=1 |
| JAR 会话恢复 | 停止 Maven 后端后启动原产物；业务和观测页面刷新均无需重新登录，业务内容及观测查询恢复 |
| 精确停止 | 用户依次停止观测端、业务前端、JAR 后端；记录的 PID 14808、31064、36392 已不存在，4080/4081/5173/5174 均释放；原 Maven PID 15640 已在切换时退出 |

身份边界证据来自正常 UI 导航与会话流程，不表述为已执行完整 HTTP 越权攻击测试。后端类迁移对旧会话的兼容方向仍为 V6 读取 V5；本轮没有可验证的真实 V5 会话，按用户已确认的方案记录限制。方面二 64 项合成兼容断言与本轮 V6 Maven 到 JAR 恢复是两类独立证据。

### 8.3 本次操作及残留

本次文章标题为 CC4C-V6-A3-20260909-144912，博客 ID 为 2097578257441521666；正文为合成用途说明和公开 Logo。每项收藏、取消、上传、提交、审核及邮件、评论、评论删除和博客删除均取得单独明确确认。评论页面未公开评论 ID，删除以本次文章、jaily、2026/09/09 15:01 和完整唯一标识内容精确定位，没有读取内部状态补取 ID。

收藏关系、本次评论和博客已通过页面撤销；不声称业务删除已物理清除上传文件、审计或消息记录。本次图片 URL 为 /blogImg/img2/0350a698ffad4c89a8d7a809e82bcf46Logo.png，上传文件保留，不直接读取或删除。原 V5 博客保留；正常浏览使其可见阅读量由 3 变为 4，这一读取副作用未通过数据库回改。未保存或删除历史草稿，进入本次编辑页时标题和正文为空且未出现历史草稿删除入口。

### 8.4 失败诊断与验收限制

- Windows PowerShell 5.1 无法满足脚本 requires，改用现有 PowerShell 7.6.5；没有降低脚本版本要求或安装工具。
- 首次启动因 Redis 连接拒绝失败；后来再次出现 Redis 无监听、liveness UP 而 readiness DOWN，用户恢复既有环境后继续。没有用清空 Session 或改源码解决。
- Prometheus 未运行时页面显示不可用；按追加授权启动后抓取恢复。部分速率/分位数及 RabbitMQ 面板仍出现暂无数据或部分数据不可用提示；不将所有 39 条查询、全部时间范围或 RabbitMQ exporter 覆盖标为通过，不为补数据修改私有抓取配置或压测。
- 核查命令曾把单行 Git 输出当作数组首项造成误报，经用户授权修正后复核通过；Actuator 响应字节数组须显式 UTF-8 解码。默认前端 HTTP 探测返回 502，显式 NoProxy 直连返回 200，未修改系统代理。
- 操作系统未提供两个前端 Node 进程的可执行文件路径，故该字段未验证；PID、启动时间、监听归属已记录，最终由用户各自终端 Ctrl+C 停止并确认 PID 消失，未执行强制进程处置。
- 未读取配置值或父进程环境验证环境恢复；已验证辅助命令的实际入口、退出流程和上传行为，不将每个环境变量恢复结果表述为动态逐项通过。

### 8.5 文档验收与方面四入口

本方面仅精确暂存两份运行及交接文档，执行现有文档链接检查、git diff --check，并逐项核对其余 381 个路径的 mode/blob、383 文件数、配置忽略及远端引用。通过后创建一个本地提交，说明为 docs: record V6 local runtime and functional verification；实际提交 SHA、唯一父提交、tree、干净状态、停止结果、记录文件 SHA-256 清单和上述限制写入本次目录中的 aspect3-result.json 及独立哈希，不 amend 嵌入自身 SHA。

方面四继续从该本地提交先制定计划：docs 最终收敛为三份扁平 Markdown，迁移并保留十三张截图和 OpenAPI，保留历史性能来源与缺失说明，整理 README 和链接。方面三不提前迁移、删除或合并资料。方面四、五、六仍未开始；不推送、创建 PR、合并 main 或打标签。
