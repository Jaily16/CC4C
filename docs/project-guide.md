...
该树表示受控职责，不等于磁盘只允许这些目录。`.git`、依赖、构建产物、运行资料和秘密目录原地保留并按忽略规则管理；没有 tracked 文件不能证明目录为空。方面六已对批准的九组候选及后代共 30 个空目录逐个非递归清理；逐项结果与保护边界见迭代总结，其他未列路径没有新增删除授权。
...
方面六在临时 Java 21、PowerShell 7 和离线 Maven 环境中完成现有静态质量门禁、后端 clean package -DskipTests 及两端 npm run build；没有安装依赖或修改版本。三种 JAR 的入口、Flyway V1–V7、catalog、根部 META-INF/spring 管理上下文导入，以及两端 dist 已核验并记录哈希。构建仍有 Shade 重复资源及大 chunk 警告，未通过改变门禁消除。

本轮用户以前台方式启动重新构建的 cc4c-6.0.0-SNAPSHOT.jar 和两端 npm dev 包裹入口。已验证四端口、三个公开健康接口、两端 HTTP、公共页面读取和未登录跳转；Prometheus 公开模板、20 条规则、外部 3.13.2 实例及后端抓取 up=1 通过。未读取终端日志或配置正文。后端进程可执行路径无法由系统接口取得，保留该核验限制。
