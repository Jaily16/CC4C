...
本轮从 README 更新合并提交 aff6df80699e1f5d1abcc924836bdfaebf86e691 开始，版本仍为 6.0.0-SNAPSHOT。新增跨平台 Node 配置与前台启动辅助；新环境自动准备独立 Pepper、消息密钥和指标密码，通过现有密码工具的标准输入生成两套 BCrypt。现有完整配置、消息密钥及外部 Prometheus 配置继续原样使用。

README 保留现有项目介绍、十张截图和架构图，删除安装教程，精简为版本范围、基本构建、必要配置和统一启动命令；性能用优化前后展示，保留原实验条件与来源。本轮删除无运行入口引用的 legacy/cc4c.sql，Flyway V1–V7 继续为初始化唯一来源；没有依赖升级或业务功能重构。

本地静态门禁和三端生产构建通过，Java 文档单元仍为 936，PowerShell 脚本仍为 18。配置与生命周期检查通过 14 组纯合成校验；独立密码工具通过 13 项标准输入／原文件兼容及 BCrypt 匹配校验；构建后的 Node 入口通过 6 项合成环境准备校验。公开与自动生成的 Prometheus 配置、20 条告警规则均通过静态检查。

本机继续使用原有完整配置，通过新 Node 入口启动三端；复用 Redis、MySQL、RabbitMQ，Prometheus 使用原私有配置和数据目录启动。三项公开健康均为 UP，两端 HTTP、首页课程与博客、公开博客图片显示正常，Prometheus 后端抓取为 1。2026-09-12 用户完成试用并明确确认正常；该确认记录为用户验收，不扩写为助手逐项重测身份或写入闭环。随后按观测端、业务前端、后端顺序向已核对身份的本次控制台发送 Ctrl+C，三个入口与应用进程均退出，四端口释放，基础设施保留运行。

本地验收已完成，GitHub 结果由本次独立 PR、准确提交对应的 Actions 及检查点确认，不提前宣称远端合并成功。Linux 和推荐版本范围未进行完整部署矩阵验证。
