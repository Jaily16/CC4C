...
| 四：文档收敛与资料迁移 | 依赖前述事实，保留证据后合并为三份文档 | 本方面收口，验收见下文 |
| 五：中文注释增强 | 以当前声明、项目指南和质量规则为准 | 未开始 |
...
本方面从上述方面三提交原地实施，只改文档、公开资产路径和 OpenAPI 扫描排除路径。独立检查点为：
...
方面五先读本总结及项目指南的代码质量章节，依据真实 AST 声明和调用关系改善中文说明；936 个 Java 文档单元和 18 个 PowerShell 脚本是方面二基线，若实际声明变化需重新核对，不机械凑数。保留原有准确注释，不改变标识符、协议、SQL、日志键和行为，不恢复测试或性能资产。超长文件表是职责审查记录，不是当前行数断言。

方面六再复核物理目录、构建／运行边界和精确 Actions；远端分支、推送、PR、合并 main、标签或发布需要相应授权。本方面没有进行这些操作。剩余两方面均未开始。
