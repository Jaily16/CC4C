...
| 2026-08-31 04:32–04:34 | `v4.0.0` 标签触发的发布工作流 `33357434615` | 失败于内置 quality/backend 的 `BusinessCacheTest.concurrentMissesUseOneLocalLoader` 第 109 行 `lockWaits` 调度断言；其余已完成质量 job 通过，container-performance/publish 未执行；[运行详情](https://github.com/Jaily16/CC4C/actions/runs/33357434615) |
| 2026-08-31 本次发布失败修复 | 将并发测试改为验证 8 次确定性 miss 与单次 loader，移除非契约 `lockWaits` 断言；本地重新运行并推送前需完成 |
