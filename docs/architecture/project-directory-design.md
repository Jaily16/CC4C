...
> 状态：方面二已落实目标结构并通过静态、生产构建与合成兼容验证；本机功能验证留在方面三，实际交接见 V6 规划第七节。
>
> 本文是 [V6 可读性迭代](../v6-iteration-plan.md)方面二的目标目录基线，已批准的精简结构继续保持。方面一、二已完成；方面三至六尚未开始。
>
> 当前源码已按本文迁移到十个技术包，实际职责见[技术分层说明](./module-boundaries.md)；此处不表示本机功能验证已完成。
...
本文已在方面一纳入 Git。原文中的“本轮只更新文档”和“后续迁移”属于规划编制时约束；方面二执行与验收以 V6 规划第七节为准，方面三才验证运行行为。
