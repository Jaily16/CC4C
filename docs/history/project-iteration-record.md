...
- [CC4C 前端功能性测试报告](frontend-functional-test-report.md)
...
- [前端功能性测试报告](frontend-functional-test-report.md)
...
- [CC4C 第二次迭代开发计划](v2/iteration-plan.md)
...
Task 1–7 的页面验收截图位于当前 Codex 任务的浏览器注释记录中，涉及 `/home`、`/allCourses`、`/allBlogs`、`/courseDetail`、`/userinfo`、`/favorite` 和 `/admin/addCourse`；未向仓库写入截图。完整 V2 回归明细见 [CC4C 前端功能性测试报告](frontend-functional-test-report.md)。
...
| 规划文档 | [CC4C 第三次迭代开发规划](v3/iteration-plan.md) |
...
- 详细技术选型、问题与收益见 [V3 技术栈升级总结](v3/technology-upgrade-summary.md)，交付证据见 [方面七报告](reports/v3/aspect7/README.md)。
...
### 13.28 V3 远程 main 与质量工作流收口

- V3 收口文档提交 `12eebc8` 已合入 `main`；首次远程质量工作流暴露两个依赖 JVM/MySQL 时区一致性的测试假设，以及旧 Trivy Action 安装器失败。
- 修复提交 `8f29872` 将过期时间改为由 Java `Instant` 显式绑定 `Timestamp`，使测试在 UTC GitHub Runner 与 Asia/Shanghai 本机上语义一致；Trivy Action 升级到固定完整 SHA，并在扫描前复用后端 Maven 缓存、预热依赖元数据。
- 本地在 `-Duser.timezone=UTC` 下复测两个目标用例通过，后端 Testcontainers `154/154`、前端两类 npm audit 和生产构建、Trivy 源码扫描均通过。
- `main` 已快进推送到 `origin/main`，二者均指向 `8f2987267a942655c1059243aaa60cf4bd29748b`。
- GitHub Actions 质量运行 [33251873844](https://github.com/Jaily16/CC4C/actions/runs/33251873844) 结论为 `success`：`backend`、`frontend`、`delivery-config`、`source-scan` 和 `compose-smoke` 全部通过；`dependency-review` 仅在 Pull Request 运行，因此本次 `main` push 按设计跳过。
- 没有创建 Git 标签，没有触发 SemVer 发布工作流，也没有发布 GHCR 镜像、SBOM、provenance 或 attestation。

## 14. 第四次迭代

### 14.1 V4 总体规划建立

- V4 以 `8f29872` 为唯一规划基线，不增加业务功能，目标是把连续迭代后的工程收敛为长期可维护形态。
- 只读盘点确认本地 `temp/` 存在大量 V2/V3 构建、扫描和临时工具副本，`target/dist/node_modules` 也属于可重建产物；这些目录均已忽略，本次仅记录现状，尚未执行删除。
- 活动仓库仍存在 `run-aspect4-benchmark.ps1`、`run-aspect6-gatling.ps1`、`Aspect2ContractFunctionalTest`、`Aspect4BenchmarkApplication` 等迭代式命名；同时 Flyway `V1–V7`、消息事件 `*.v1`、历史报告和 `database/legacy` 属于兼容或历史资产，不能按名称误删。
- 当前版本说明分散在 POM、npm、Dockerfile、Compose、脚本、Actions 和 README；V4 将建立唯一当前版本基线及自动一致性校验，而不是删除各工具必需的机器声明。
- 当前没有根级 EditorConfig、Java 格式化或前端 ESLint/Prettier 配置；已识别超大缓存/消息/领域服务和 Vue 页面，后续只在行为测试保护下按职责拆分或合并。
- V4 划分为六个依次实施的方面：基线盘点与安全清理、版本/依赖/项目身份统一、仓库与源码结构规范化、代码风格/中文注释/内部整合、容器与宿主机双运行模式、全量验证/文档收口/发布准备。
- 总体范围、安全边界、目标目录、兼容风险、验收门禁和新对话的前两个提示词见 [CC4C 第四次迭代开发规划](../development/v4-iteration-plan.md)。六方面当前均为“未实施”，后续每个方面需在新对话中先使用计划模式制定详细实施计划。
