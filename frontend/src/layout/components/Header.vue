  ...
  <nav class="top-header" aria-label="主导航">
    ...
    <div class="top-header__links">
      <router-link v-for="item in navItems" :key="item.to" v-slot="{ isExactActive, navigate }" :to="item.to" custom>
        <button type="button" class="nav-link" :class="{ 'nav-link--active': isExactActive }" @click="navigate">
          {{ item.label }}
        </button>
      </router-link>
    </div>
    ...
    <div class="top-header__actions">
      <el-button v-if="isLoggedIn" class="session-action" size="small" :icon="SwitchButton" @click="logout">
        退出登录
      </el-button>
      <el-button v-else class="session-action" size="small" type="primary" :icon="Position" @click="flyToLogin">
        登录
      </el-button>
    </div>
  </nav>
...
const { isUser: isLoggedIn } = useCurrentUser();
const navItems = [
  { label: '主页', to: '/home' },
...
async function logout() {
  try {
    ...
    reportClientError(error, 'frontend/src/layout/components/Header.vue');
  }
}
