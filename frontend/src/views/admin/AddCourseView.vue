  ...
  <section class="course-publisher" aria-labelledby="course-publisher-title">
    ...
    <form class="publisher-form" @submit.prevent="publishCourse">
      <CourseBasicsForm
        ...
        @update-field="updateCourseFormField"
      ...
      <CourseModuleEditor
        ...
        :languages="languages"
        :dialog-open="moduleDialogOpen"
        :module-form="moduleForm"
        :module-name-error="moduleNameError"
        :module-levels="moduleLevels"
        :module-submitting="moduleSubmitting"
        @update:dialog-open="moduleDialogOpen = $event"
        @open="openModuleDialog"
        @load="loadModules"
        @reset="resetModuleForm"
        @add="addModule"
        @update-module-field="updateModuleFormField"
        @clear-error="clearFormError"
        @clear-module-error="moduleNameError = ''"
      />
      ...
      <section class="publisher-card" aria-labelledby="course-content-title">
        <header class="step-heading">
          <span>03</span>
          <div>
            <h2 id="course-content-title">课程正文 *</h2>
            <p>支持 Markdown 和图片，请在提交前检查内容结构。</p>
          </div>
          <small
            ><el-icon v-if="uploading" class="is-loading"><Loading /></el-icon>{{ editorStatus }}</small
          >
        </header>
      ...
      <footer class="publisher-actions">
        ...
        <el-button native-type="submit" type="primary" size="large" :loading="publishing" :disabled="publishDisabled"
          >发布新课程</el-button
        >
      </footer>
    </form>
...
import { uploadBlogImage } from '@/api/community';
...
const modules = ref(
  languages.map((language) => ({ label: language.label, value: language.name, disabled: true, children: [] })),
);
...
async function loadModules() {
  ...
  moduleError.value = '';
  try {
    const responses = await Promise.all(languages.map((language) => listModules(language.value)));
    ...
    modules.value = languages.map((language) => ({
      label: language.label,
      value: language.name,
      disabled: true,
      children: [],
    }));
    reportClientError(error, 'frontend/src/views/admin/AddCourseView.vue');
  } finally {
...
async function onUploadImg(files, callback) {
  ...
  try {
    ...
    ElMessage.error(apiErrorMessage(error, error.message || '图片上传失败，请稍后重试'));
    reportClientError(error, 'frontend/src/views/admin/AddCourseView.vue');
  } finally {
...
.required-note {
  ...
  border-radius: 14px;
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.96), #f1f6ff);
...
.publisher-actions div {
  display: grid;
  gap: 4px;
}
.publisher-actions strong {
  color: var(--cc4c-text);
}
