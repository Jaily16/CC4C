...
/** 提交管理员凭据，成功后强制恢复业务 Session，再跳转指定管理路由或内容页。 */
async function login() {
  ...
  try {
    ...
    const redirect = typeof route.query.redirect === 'string' ? route.query.redirect : '/admin/CoursesAndBlogs';
    await router.push(redirect);
  } catch (error) {
    formError.value = apiErrorMessage(error, '管理员登录服务暂时不可用，请稍后重试');
    ElMessage.error(formError.value);
    reportClientError(error, 'frontend/src/views/admin/AdminLoginView.vue');
  } finally {
...
.auth-brand > p:not(.auth-brand__eyebrow) {
  max-width: 460px;
