          ...
          >
            <div class="course-card__visual">
              <img
                ...
                :src="assets.languageCards[course.languageName]"
                :alt="`${course.languageName || '课程'}课程`"
              />
...
async function requestCourses() {
  ...
  try {
    ...
    const resp = isSearching.value
      ? await searchCoursesApi(searchInfo.value, params)
      : await listCoursesByLanguage(mainLang.value, params);
    ...
    reportClientError(error, 'frontend/src/views/course/AllCoursesView.vue');
  } finally {
    loading.value = false;
  }
}
...
.discovery-hero {
  ...
  border-radius: 20px;
  background: var(--cc4c-discovery-hero);
  color: white;
...
.discovery-hero p:not(.discovery-hero__eyebrow) {
  max-width: 660px;
  margin: 14px 0 0;
  color: #dbeafe;
  line-height: 1.65;
...
.discovery-hero__stat strong {
  font-size: 2rem;
  line-height: 1;
}
.discovery-hero__stat span {
  margin-top: 6px;
  color: #dbeafe;
  font-size: 0.8125rem;
}
...
.language-picker :deep(.el-radio-button:first-child .el-radio-button__inner),
.language-picker :deep(.el-radio-button:last-child .el-radio-button__inner) {
  border-radius: 9px !important;
}
.language-option {
  display: inline-flex;
...
.discovery-search :deep(.el-input) {
  min-width: 0;
  flex: 1;
}
.collection-heading {
...
.course-card:focus-visible .course-card__image {
