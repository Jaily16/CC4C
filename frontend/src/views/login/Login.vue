...
import { loginUser, requestVerificationCode, resetPassword } from '@/api/auth';
...
async function login() {
  ...
  try {
    ...
    ElMessage.error(formError.value);
    reportClientError(error, 'frontend/src/views/login/Login.vue');
  } finally {
...
async function getVCode() {
  ...
  try {
    const resp = await requestRecoveryCode();
    if (!resp) return;
    if (!resp.data.data) {
