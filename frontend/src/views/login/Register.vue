        ...
        <el-form class="auth-form" label-position="top" @submit.prevent="register">
          ...
          <el-form-item label="邮箱验证码 *" :error="fieldErrors.code">
            <div class="verification-row">
              ...
              />
              <el-button type="primary" plain :loading="sendingCode" :disabled="countdown > 0" @click="getVCode">
...
const {
  ...
  requestCode: requestVerificationCode,
...
async function getVCode() {
  ...
  try {
    const resp = await requestRegisterCode();
    if (!resp) return;
    if (!resp.data.data) {
    ...
    ElMessage.error(formError.value);
    reportClientError(error, 'frontend/src/views/login/Register.vue');
  }
}
...
.auth-form__choices :deep(.el-select) {
  width: 100%;
}
.verification-row {
  display: flex;
