...
async function loadCourses() {
  ...
  coursesError.value = '';
  try {
    const resp = await listHomeCourses({ page: 1, size: 8 });
    courses.value = resp.data.data?.items || [];
  } catch (error) {
    courses.value = [];
    coursesError.value = apiErrorMessage(error, '课程推荐加载失败，请检查网络后重试。');
    reportClientError(error, 'frontend/src/views/HomeView.vue');
  } finally {
...
async function loadBlogs() {
  ...
  blogsError.value = '';
  try {
    const resp = await listHomeBlogs({ page: 1, size: 6 });
...
.welcome-panel {
  ...
  border-radius: calc(var(--cc4c-radius) + 6px);
...
.partner-carousel :deep(.el-carousel__container) {
  height: clamp(180px, 35vw, 350px) !important;
}
.partner-carousel :deep(.el-image) {
  width: 100%;
  height: 100%;
}
.course-grid {
...
.blog-row {
  ...
  transition:
    ...
    background-color var(--cc4c-transition);
}
.course-card {
  overflow: hidden;
  border: 1px solid var(--cc4c-border);
...
.course-card__language {
  ...
  font-size: 0.8125rem;
  font-weight: 700;
  transition: color var(--cc4c-transition);
}
.course-card h3,
.blog-row h3 {
  margin: 0;
...
.course-card:hover .course-card__language,
.course-card:focus-visible .course-card__language {
  color: #1e40af;
}
