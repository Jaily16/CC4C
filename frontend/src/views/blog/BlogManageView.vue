    ...
    <div class="blog-manage__content">
      ...
      <section class="manage-card" aria-labelledby="my-blogs-title">
        ...
        <div v-if="blogList.length" class="status-summary" aria-label="博客状态统计">
          <span
            ><strong>{{ total }}</strong> 全部</span
          >
          <span
            ><strong>{{ stateCounts.published }}</strong> 已发布</span
          >
          <span
            ><strong>{{ stateCounts.pending }}</strong> 待审核</span
          >
          <span
            ><strong>{{ stateCounts.rejected }}</strong> 未通过</span
          >
        </div>
        ...
        <PageFeedback :loading="loading" :error="errorMessage" @retry="loadBlogs">
          ...
          <div v-else class="blog-list">
            ...
            >
              <div
                class="blog-item__marker"
                :class="`blog-item__marker--${statusInfo(blog.state).key}`"
                aria-hidden="true"
              ></div>
              <div class="blog-item__body">
                <div class="blog-item__topline">
                  <el-tag :type="statusInfo(blog.state).type" effect="light">{{ statusInfo(blog.state).label }}</el-tag>
                  <time v-if="blog.publishTime" :datetime="String(blog.publishTime)">{{
                ...
                <div class="blog-item__meta">
                  <span v-if="blog.click !== undefined"
                    ><el-icon><View /></el-icon>{{ blog.click }} 次阅读</span
                  >
                  <span
                    ><el-icon><Document /></el-icon>博客文章</span
                  >
                </div>
              </div>
  ...
  return (
    {
      '-1': {
        ...
        label: '审核未通过',
        type: 'danger',
        description: '文章未通过审核，可以调整内容后重新撰写并提交。',
      },
      ...
      1: {
        ...
        label: '已发布',
        type: 'success',
        description: '文章已公开发布，社区用户可以阅读和评论。',
      },
...
async function loadBlogs() {
  ...
  try {
    ...
    if (!verified) return;
    const resp = await listMyBlogs({ page: currentPage.value, size: pageSize });
...
.manage-heading p:not(.manage-heading__eyebrow) {
  margin: 8px 0 0;
...
.blog-item__marker--published {
  background: #22c55e;
}
.blog-item__marker--pending {
  background: #f59e0b;
}
.blog-item__marker--rejected {
  background: #ef4444;
}
.blog-item__body {
  min-width: 0;
...
.blog-item__actions :deep(.el-button) {
  margin: 0;
}
.manage-pagination {
