            ...
            <div class="basic-grid">
              ...
              <fieldset class="form-field language-field">
                ...
                <el-checkbox-group v-model="langList" class="language-options" @change="languageError = ''">
                  <el-checkbox v-for="lang in languages" :key="lang.value" :label="lang.value">{{
                    lang.label
                  }}</el-checkbox>
                </el-checkbox-group>
          ...
          <footer class="editor-actions">
            ...
            <div>
              <el-button
                ...
                type="danger"
                plain
                :disabled="draftSaving || publishSubmitting || uploading"
                @click="deleteDraft"
                >删除草稿</el-button
              >
              <el-button size="large" :loading="draftSaving" :disabled="publishSubmitting || uploading" @click="draft"
                >保存草稿</el-button
              >
              <el-button
                native-type="submit"
...
import { getDraft, removeDraft, saveDraft, submitBlog, uploadBlogImage } from '@/api/community';
...
async function loadDraft() {
  try {
    ...
    reportClientError(error, 'frontend/src/views/blog/BlogWriteView.vue');
  }
}
  ...
  try {
    const responses = await Promise.all(
      files.map(async (file) => {
        ...
        const resp = await uploadBlogImage(form);
        if (!resp.data?.url) {
...
.editor-section__heading > div:first-child {
  display: flex;
  gap: 10px;
  align-items: center;
}
.editor-section__heading > div:first-child > span {
  display: grid;
  width: 30px;
  height: 30px;
  place-items: center;
...
.editor-section__heading small,
.field-help {
  color: var(--cc4c-muted);
}
.editor-section__heading b,
.form-field b {
...
.editor-actions {
  ...
  align-items: center;
  justify-content: space-between;
  padding: 22px clamp(20px, 3.5vw, 36px);
  background: #f8fafc;
}
.editor-actions p {
  max-width: 620px;
