...
import { incrementBlogClick } from '@/api/community';
...
function courseImage(languageName) {
  return (
    ...
    assets.languageCards[String(languageName || '').toLowerCase()] ||
...
async function loadFavorites() {
  ...
  try {
    ...
    const [courseResponse, blogResponse] = await Promise.all([
      listCourseFavorites({ page: coursePage.value, size: pageSize }),
      listBlogFavorites({ page: blogPage.value, size: pageSize }),
    ]);
    ...
    reportClientError(error, 'frontend/src/views/FavoriteView.vue');
  } finally {
    loading.value = false;
  }
}
...
.favorite-tabs button:hover {
  color: var(--cc4c-primary);
}
.favorite-tabs button b {
  min-width: 25px;
  padding: 2px 7px;
  border-radius: 999px;
  background: #e2e8f0;
...
.favorite-tabs .favorite-tabs__button--active {
  border-color: #cad9f3;
  background: #fff;
  color: var(--cc4c-primary);
...
.favorite-tabs .favorite-tabs__button--active b {
  background: #dbeafe;
}
.favorite-center .page-feedback {
  position: relative;
  z-index: 1;
}
.favorite-panel {
...
.favorite-center .course-grid {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 18px;
}
.favorite-center .course-card {
  min-width: 0;
  padding: 0;
  overflow: hidden;
  border: 1px solid var(--cc4c-border);
...
.favorite-center .course-card:hover {
  border-color: #93b4ef;
  transform: translateY(-4px);
  box-shadow: 0 15px 28px rgba(37, 99, 235, 0.12);
}
.favorite-center .course-card__visual {
  position: relative;
  display: block;
  height: 154px;
  overflow: hidden;
...
.favorite-center .course-card__visual img {
...
.favorite-center .course-card:hover .course-card__visual img {
  transform: scale(1.045);
}
.favorite-center .course-card__visual small {
  position: absolute;
  top: 12px;
  left: 12px;
  padding: 4px 9px;
...
.favorite-center .course-card__body {
  display: grid;
  gap: 13px;
  padding: 18px;
  border-top: 1px solid #edf1f7;
}
.favorite-center .course-card__body b {
  min-height: 2.8em;
  color: var(--cc4c-text);
  line-height: 1.4;
  overflow-wrap: anywhere;
}
.favorite-center .course-card__body span {
  color: var(--cc4c-primary);
  font-size: 0.86rem;
  font-weight: 700;
}
.favorite-center .course-card__body i {
  font-style: normal;
  transition: transform var(--cc4c-transition);
}
.favorite-center .course-card:hover .course-card__body i {
  display: inline-block;
  transform: translateX(4px);
}
.favorite-center .blog-list {
  display: grid;
  gap: 12px;
}
.favorite-center .blog-card {
  display: grid;
...
.favorite-center .blog-card:hover {
  border-color: #93b4ef;
  transform: translateY(-2px);
  box-shadow: 0 12px 26px rgba(37, 99, 235, 0.1);
}
.favorite-center .blog-card__marker {
  display: grid;
  place-items: center;
  width: 54px;
  height: 54px;
...
.favorite-center .blog-card__content {
  display: grid;
  gap: 5px;
  min-width: 0;
}
.favorite-center .blog-card__content small,
.favorite-center .blog-card__content > span {
  color: var(--cc4c-muted);
  font-size: 0.78rem;
}
.favorite-center .blog-card__content b {
  color: var(--cc4c-text);
  font-size: 1rem;
  overflow-wrap: anywhere;
}
.favorite-center .blog-card > i {
  color: var(--cc4c-primary);
  font-size: 1.25rem;
  font-style: normal;
}
