...
try {
    ...
    if ($null -eq $state -or $state.status -eq 'stopped') {
        Write-Output 'No running recorded frontend requires stopping.'
        exit 0
    }
    if ($state.status -ne 'running' -or $state.component -ne 'frontend') { throw 'Unexpected application state; nothing was stopped.' }
    Stop-Cc4cOwnedComponent $state
    Write-Output "Recorded frontend PID $($state.pid) is stopped; external services were unchanged."
    exit 0
}
catch {
