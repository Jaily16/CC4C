...
const snapshotPath = resolve(process.argv[3] || 'docs/reference/openapi.json')
const sourceUrl = process.env.CC4C_OPENAPI_URL || 'http://127.0.0.1:4080/v3/api-docs'

if (!['check', 'update'].includes(mode)) {
  throw new Error('Usage: node scripts/testing/openapi-snapshot.mjs check|update [snapshot]')
}
