# 运行前提：只在隔离测试数据库上执行，并由调用方明确提供非敏感工具路径。
# 破坏性边界：仅准备 Flyway 测试资源；不得执行 Flyway clean/repair，不触碰生产数据或现有卷。
# 失败恢复：失败时保留备份与日志，停止后续迁移；按备份校验清单人工恢复。
# 退出码：准备成功返回 0，参数、备份或连接失败返回非零码。
...
$backupDirectory = Join-Path $workspaceRoot 'temp'
$backupDirectory = [System.IO.Path]::GetFullPath($backupDirectory)
[System.IO.Directory]::CreateDirectory($backupDirectory) | Out-Null
$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupPath = Join-Path $backupDirectory "cc4c-flyway-test-$($main.Database)-$timestamp.sql"
