# 运行前提：必须在隔离性能数据库上运行，并提供明确的 Before/After 阶段和只读查询工具。
# 破坏性边界：只采集 EXPLAIN 结果，不修改业务数据，不触碰生产卷、上传文件或秘密内容。
# 失败恢复：采集失败时保留已生成的非敏感输出并停止，不覆盖既有报告。
# 退出码：采集成功返回 0，参数、连接或查询失败返回非零码。
...
$outputPath = Join-Path $outputDirectory "cc4c-performance-explain-$($Phase.ToLowerInvariant()).txt"
$queries = [ordered]@{
    course_home = @'
...
"@
    $performanceIndexCount = [int] (& $mysql @commonArguments "--execute=$performanceIndexCountSql")
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to inspect aspect four query indexes."
    }
    if ($Phase -eq 'Before' -and $performanceIndexCount -ne 0) {
        throw "Before plans require the pre-V5 schema."
    }
    if ($Phase -eq 'After' -and $performanceIndexCount -ne 2) {
