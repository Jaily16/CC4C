...
# 运行前提：使用当前工作区的栈记录及三端完整身份快照，不加载任何环境文件。
# 外部依赖：仅依赖三端 stop 脚本、共享进程身份函数和 Windows 进程创建时间元数据。
# 破坏性边界：只按观测端、业务前端、后端顺序停止精确 PID，不触及外部服务或数据。
...
try {
    ...
    Stop-Cc4cOwnedComponent $stack.observability
    Write-Output "Observability PID $($stack.observability.pid) is stopped."
    ...
    Write-Output 'Only the three recorded applications were stopped. All external services and data were preserved.'
    exit 0
}
catch {
