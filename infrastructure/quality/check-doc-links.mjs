...
// 只检查 Git 清单内源文件及父路径元数据，拒绝链接/硬链接，避免受控路径被重定向到本机秘密。
async function assertOrdinarySource(repositoryRoot, absolute) {
  let cursor = absolute;
  while (true) {
    const metadata = await lstat(cursor);
    const file = cursor === absolute;
    if (metadata.isSymbolicLink() || (file ? !metadata.isFile() || metadata.nlink !== 1 : !metadata.isDirectory())) {
      throw new Error('A controlled source path is redirected or has an unsafe file type.');
    }
    const actual = await realpath(cursor);
    const normalize = (value) => process.platform === 'win32' ? value.toLowerCase() : value;
    if (normalize(actual) !== normalize(cursor)) throw new Error('A controlled source path was redirected.');
    if (cursor === repositoryRoot) return;
    cursor = path.dirname(cursor);
  }
}

// 只以 Git 返回的清单选择文档，不递归遍历工作区。
async function gitPaths(repositoryRoot, args) {
  try {
    ...
    throw new Error('Git file inventory is unavailable; no filesystem fallback or documentation scan was performed.')
  }
}
...
export async function loadDocLinkSnapshot(repositoryRoot) {
  ...
  const deletedPaths = new Set(await gitPaths(root, ['ls-files', '--deleted', '-z']))
  const repositoryPaths = [...new Set([...trackedPaths, ...untrackedPaths])]
    .filter((relative) => !deletedPaths.has(relative))
  ...
  for (const relative of files) {
    ...
    await assertOrdinarySource(root, absolute)
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  try { await main() } catch (error) {
    process.stderr.write(`${error?.message || 'Documentation link check failed'}\n`)
    process.exitCode = 1
  }
}
