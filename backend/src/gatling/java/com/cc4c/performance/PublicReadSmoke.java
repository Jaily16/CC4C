...
public final class PublicReadSmoke extends Simulation {
    private final ScenarioBuilder scenario =
            scenario("PublicReadSmoke").during(Duration.ofMinutes(1)).on(PerformanceSupport.publicRead());

    public PublicReadSmoke() {
        setUp(scenario.injectClosed(constantConcurrentUsers(20).during(Duration.ofMinutes(1))))
