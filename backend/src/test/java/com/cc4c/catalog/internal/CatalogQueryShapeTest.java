...
class CatalogQueryShapeTest {
    ...
    void modulesUseOneModuleQueryAndOneBulkCourseQuery() {
        ...
        when(mapper.selectCourseNamesByLanguage(7))
                .thenReturn(List.of(courseName(1, "first"), courseName(1, "second"), courseName(2, "third")));
    ...
    void recommendationUsesOneModuleQueryAndOneBulkCourseQuery() {
        CatalogMapper mapper = mock(CatalogMapper.class);
        CatalogService service = new CatalogService(mapper, disabledCache());
        when(mapper.selectModulesForRecommendation(7, 0, 1)).thenReturn(List.of(module(7, 3), module(7, 4)));
        when(mapper.selectRecommendedCourseNamesByLanguage(7, 1, 2))
                .thenReturn(List.of(courseName(3, "recommended-a"), courseName(4, "recommended-b")));
