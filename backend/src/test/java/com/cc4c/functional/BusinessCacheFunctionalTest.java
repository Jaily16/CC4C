...
class BusinessCacheFunctionalTest extends FunctionalTestSupport {
    ...
    void verifiedBlogDetailIsCachedButPendingBlogRemainsPrivate() throws Exception {
        ...
        mockMvc.perform(get("/blogs/{id}", verified.id())).andExpect(status().isOk());
        mockMvc.perform(get("/blogs/{id}", verified.id())).andExpect(status().isOk());
        mockMvc.perform(get("/blogs/{id}", pending.id()).with(asUser(writer))).andExpect(status().isOk());
        mockMvc.perform(get("/blogs/{id}", pending.id()).with(anonymous())).andExpect(status().isNotFound());
