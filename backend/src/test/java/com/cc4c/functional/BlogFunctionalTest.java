...
class BlogFunctionalTest extends FunctionalTestSupport {
    ...
    void blogFavoritesUsePostConflictAndIndependentPagination() throws Exception {
        ...
        mockMvc.perform(get("/blogs/collect")
                        .with(asUser(user))
                        .param("page", "1")
                        .param("size", "8"))
                .andExpect(status().isOk())
    ...
    void draftUpsertIsNonDestructiveAndPublishingClearsIt() throws Exception {
        ...
        mockMvc.perform(post("/blogs/submit")
                        ...
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(blogPayload(unique("draft_publish_"), language.id()))))
                .andExpect(status().isCreated());
        mockMvc.perform(get("/blogs/draft").with(asUser(user)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").doesNotExist());

        mockMvc.perform(delete("/blogs/draft").with(asUser(user)).with(csrf())).andExpect(status().isOk());
    }
    ...
    void invalidBlogReferencesNeverCreatePartialWrites() throws Exception {
        ...
        mockMvc.perform(post("/blogs/submit")
                        ...
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(blogPayload(invalidTitle, 999999))))
                .andExpect(status().isUnprocessableEntity());
        assertEquals(
                0,
                jdbcTemplate.queryForObject("SELECT COUNT(*) FROM blog WHERE title = ?", Integer.class, invalidTitle));

        mockMvc.perform(post("/blogs/collect/{blogId}", 999999999999L)
                        .with(asUser(writer))
                        .with(csrf()))
                .andExpect(status().isNotFound());
        mockMvc.perform(put("/blogs/click/{id}", 999999999999L).with(csrf())).andExpect(status().isNotFound());
    ...
    void blogImageUploadKeepsEditorStringContract() throws Exception {
        ...
        MockMultipartFile image =
                new MockMultipartFile("file", "article.png", MediaType.IMAGE_PNG_VALUE, new byte[] {1, 2, 3});

        mockMvc.perform(multipart("/blogs/uploadImg")
                        .file(image)
                        .with(asUser(user))
                        .with(csrf()))
                .andExpect(status().isOk())
    ...
    void onlyTheBlogAuthorCanDeleteIt() throws Exception {
        ...
        mockMvc.perform(delete("/blogs/delete")
                        .param("blogId", Long.toString(blog.id()))
                        .with(asUser(other))
                        .with(csrf()))
        ...
        mockMvc.perform(delete("/blogs/delete")
                ...
                .andExpect(status().isOk());
        assertEquals(
                1, jdbcTemplate.queryForObject("SELECT deleted FROM blog WHERE blog_id = ?", Integer.class, blog.id()));
    }

    private Map<String, Object> blogPayload(String title, int languageId) {
        return Map.of("title", title, "content", "Functional blog content", "languageList", List.of(languageId));
    }
}
