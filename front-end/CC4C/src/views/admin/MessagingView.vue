      ...
      <el-table v-if="messages.length" :data="messages" stripe>
        ...
        <el-table-column label="操作" width="170" fixed="right">
          <template #default="scope">
            <el-button link type="primary" :disabled="!scope.row.recoverable" @click="retryMessage(scope.row)">重试</el-button>
            <el-button link type="danger" :disabled="!scope.row.recoverable" @click="ignoreMessage(scope.row)">忽略</el-button>
          </template>
        </el-table-column>
      </el-table>
...
async function loadMessages() {
  ...
  errorMessage.value = '';
  try {
    const response = await axios.get('/admin/messaging/messages', {
      params: {
        page: page.value,
        size,
...
async function retryMessage(row) {
  await ElMessageBox.confirm('确认重新投递这条消息？外部邮件服务处于不确定状态时可能收到内容相同的邮件。', '重试消息', { type: 'warning' });
  try {
    await axios.post(`/admin/messaging/messages/${row.eventId}/retry`);
...
async function ignoreMessage(row) {
  ...
  try {
    await axios.post(`/admin/messaging/messages/${row.eventId}/ignore`);
...
<style scoped>
.messaging-page { display: grid; gap: 22px; width: min(100%, 1380px); margin: 0 auto; }
.page-heading { display: flex; gap: 20px; align-items: flex-end; justify-content: space-between; }
.page-heading p { margin: 0 0 5px; color: var(--cc4c-primary); font-size: .72rem; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; }
.page-heading h1 { margin: 0; color: var(--cc4c-text); font-size: clamp(1.65rem, 3vw, 2.25rem); }
.page-heading span { display: block; margin-top: 8px; color: var(--cc4c-muted); }
.filters { display: flex; flex-wrap: wrap; gap: 12px; }
.filters .el-select { width: min(100%, 290px); }
.message-panel { display: grid; gap: 18px; padding: 20px; border: 1px solid var(--cc4c-border); border-radius: 16px; background: #fff; box-shadow: 0 8px 24px rgba(15, 23, 42, .05); }
.message-panel .el-pagination { justify-self: end; }
@media (max-width: 640px) { .page-heading { align-items: start; flex-direction: column; } .message-panel { padding: 12px; overflow-x: auto; } }
</style>
