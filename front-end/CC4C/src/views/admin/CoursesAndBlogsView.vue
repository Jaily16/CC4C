        ...
        <div v-if="courseList.length" class="data-table-scroll" tabindex="0" aria-label="课程数据表，可横向滚动">
          <el-table :data="courseList" stripe>
            ...
            <el-table-column prop="languageName" label="语言" width="100">
              <template #default="scope"><el-tag effect="plain">{{ scope.row.languageName || '未分类' }}</el-tag></template>
            </el-table-column>
            ...
            <el-table-column label="状态" width="100">
              <template #default><span class="status-dot"><i></i>已发布</span></template>
            </el-table-column>
          </el-table>
        </div>
        ...
        <div v-if="blogList.length" class="data-table-scroll" tabindex="0" aria-label="博客数据表，可横向滚动">
          <el-table :data="blogList" stripe>
            ...
            <el-table-column label="状态" width="110">
              <template #default><span class="status-dot"><i></i>已公开</span></template>
            </el-table-column>
          </el-table>
        </div>
...
<style scoped>
.admin-page { display: grid; gap: 24px; width: min(100%, 1380px); margin: 0 auto; }
.admin-page__heading { display: flex; gap: 24px; align-items: flex-end; justify-content: space-between; }
.admin-page__heading p { margin: 0 0 5px; color: var(--cc4c-primary); font-size: .72rem; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; }
.admin-page__heading h1 { margin: 0; color: var(--cc4c-text); font-size: clamp(1.65rem, 3vw, 2.25rem); }
.admin-page__heading span { display: block; margin-top: 8px; color: var(--cc4c-muted); }
.overview-stats { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 16px; }
.overview-stats article { display: grid; gap: 5px; padding: 22px; border: 1px solid var(--cc4c-border); border-radius: 16px; background: #fff; box-shadow: 0 8px 24px rgba(15, 23, 42, .05); }
.overview-stats span { color: var(--cc4c-muted); font-size: .78rem; font-weight: 700; }
.overview-stats strong { color: var(--cc4c-text); font-size: 2rem; line-height: 1.2; }
.overview-stats article:first-child strong { color: var(--cc4c-primary); }
.overview-stats small { color: #94a3b8; }
.overview-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 20px; align-items: start; }
.data-panel { min-width: 0; overflow: hidden; border: 1px solid var(--cc4c-border); border-radius: 17px; background: #fff; box-shadow: 0 10px 28px rgba(15, 23, 42, .055); }
.data-panel__heading { display: flex; gap: 16px; align-items: center; justify-content: space-between; padding: 20px 22px; border-bottom: 1px solid var(--cc4c-border); }
.data-panel__heading h2 { margin: 0; color: var(--cc4c-text); font-size: 1.08rem; }
.data-panel__heading p { margin: 4px 0 0; color: var(--cc4c-muted); font-size: .78rem; }
.data-table-scroll { width: 100%; overflow-x: auto; }
.data-table-scroll :deep(.el-table) { min-width: 720px; }
.data-table-scroll :deep(.el-table__header th) { background: #f8fafc; color: #526079; }
.status-dot { display: inline-flex; gap: 7px; align-items: center; color: #15803d; font-size: .78rem; font-weight: 700; }
.status-dot i { width: 7px; height: 7px; border-radius: 50%; background: #22c55e; }

@media (max-width: 1180px) { .overview-grid { grid-template-columns: 1fr; } }
@media (max-width: 700px) {
  .admin-page__heading { align-items: flex-start; flex-direction: column; }
  .overview-stats { grid-template-columns: 1fr; }
  .admin-page__heading .el-button { width: 100%; }
}
</style>
